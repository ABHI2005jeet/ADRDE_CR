...
export default function DashboardPage() {
  ...
  const completedAgendas = agendas.filter((agenda) => agenda.status === 'Completed').length;
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        description="Operational overview for meeting agenda management, document flow, and action tracking."
        eyebrow="MAC control room"
        title="ADRDE Agra - MAC Meeting Agenda Dashboard"
      />

      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <StatCard helper="Scheduled from mock data" icon="calendar" label="Upcoming Meetings" tone="navy" value={upcomingMeetings} />
        <StatCard helper="Across all MAC meetings" icon="agenda" label="Total Agendas" tone="blue" value={agendas.length} />
        <StatCard helper="Unique departments and groups" icon="users" label="Participants" tone="green" value={participants} />
        <StatCard helper="PDF, DOC, image records" icon="document" label="Uploaded Documents" tone="slate" value={documents.length} />
